...
def findTarget(img, White, Red):
    Wx1,Wy1,Wx2,Wy2,Wxc,Wyc,img = findColor(White)
    Rx1,Ry1,Rx2,Ry2,Rxc,Ryc,img = findColor(Red)
    if(Wx1<Rxc<Wy1):
        if(Wy1<Ryc<Wy2):
            xTarget = Wxc
            yTarget = Wyc
        ...
        xTarget = 320
        yTarget = 220
    return xTarget, yTarget, img

def findColor(img,Color):
    #converts image to HSV color spectrum, less sensitive to lighting as compared to RGB
	imgHSV = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
	lower = np.array([Color[0],Color[1],Color[2]])
	upper = np.array([Color[3],Color[4],Color[5]])
	mask = cv2.inRange(imgHSV,lower,upper)
	x1,y1,x2,y2,xc,yc,img=getContours(img,mask)
	cv2.circle(img,(xc,yc),10,(255,0,0),cv2.FILLED)
    #slaps a beautiful blue circle onto the detected object
	return x1,y1,x2,y2,xc,yc,img
...
def getContours(img,mask):
	...
	return x,y,x+w,y+h,x+w//2,y+h//2,img
...
def trackLand():
    ...
    while True:
        success,img = cap.read()
        imgResult = img.copy()
        x,y,imgResult = findTarget(img, White, Red)
